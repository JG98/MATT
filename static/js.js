$(function() {
    ...
    function load(method, httpData) {
        if (method == "post") {
            $.post("load", httpData, draw);
        } else {
            $.get("load", httpData, draw);
        }
    }
    ...
    function draw(data, status){
        ...
        $(svg.node).appendTo($("#mainDiv"));
        g = svg.g();
        g.add(svg.path("M0,0H" + maxX + "V" + maxY + "H0V0").attr({fill: 'none', stroke: 'black', strokeWidth: 2}));
        ...
        topBottom.forEach(function(value) {
            for (var i = 0; i < maxDistance * 10; i++) {
                topBottomLine = svg.line(10 + scaleX * i / 10, value - 10, 10 + scaleX * i / 10, value).attr({fill: 'none', stroke: 'black', strokeWidth: 2});
            ...
            topBottomLine = svg.line(10 + scaleX * maxDistance, value - 10, 10 + scaleX * maxDistance, value).attr({fill: 'none', stroke: 'black', strokeWidth: 2});
            ...
            g.add(svg.line(10, value - 5, 10 + scaleX * maxDistance, value - 5).attr({fill: 'none', stroke: 'black', strokeWidth: 2}));
        });
        for (var i = 0; i < maxDistance * 10; i++) {
            topBottomLine = svg.line(10 + scaleX * i / 10, topBottom[0], 10 + scaleX * i / 10, topBottom[1] - 10).attr({fill: 'none', stroke: 'black', strokeWidth: 2, strokeOpacity: 0.125});
            topBottomLine.append(Snap.parse('<title>' + i / 10 + '</title>'));
            g.add(topBottomLine);
        }
        topBottomLine = svg.line(10 + scaleX * maxDistance, topBottom[0], 10 + scaleX * maxDistance, topBottom[1] - 10).attr({fill: 'none', stroke: 'black', strokeWidth: 2, strokeOpacity: 0.125});
        topBottomLine.append(Snap.parse('<title>' + maxDistance + '</title>'));
        g.add(topBottomLine);
        ...
        data.forEach(function(item, index, array){
            if (item["name"] != "None") {
                ...
                if (item["bootstrap"] != "None" && item["bootstrap"] != "") {
                    ...
                    g.add(svg.text((parseFloat(array[parent]["total_distance"]) + (item["distance"] / 2)) * scaleX, (index + 1) * scaleY, item["bootstrap"]).attr({dominantBaseline: 'baseline', fontSize: 0.5 * fontSize, 'data-id': item["id"]}));
        ...
        function setTransform() {
            ...
            if (arguments[0] == "translate") {
                ...
                scale = arguments[1];
                translateX = 100 * scale; //arguments[2]; // TODO
                translateY = 100 * scale; //arguments[3]; // TODO
